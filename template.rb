...
# $ rails new appname -m templates/template.rb
# $ cd appname
# $ gem install bundler
# $ bundle install

# Old - create rvmrc file
#create_file ".rvmrc", "rvm use 1.9.2@#{app_name}"
#run "echo 'rvm use 1.9.2@#{app_name}' > .rvmrc"
...
#Compass
run ("compass init --app rails --using blueprint")
...
copy_file(screen_path, "app/stylesheets/screen.scss")
copy_file(base_path, "app/stylesheets/partials/_base.scss")
...
#inject_into_file 'config/routes.rb', "\n# Added via my Template\n", :before => "# The priority is based upon order of creation:"
#inject_into_file 'config/routes.rb', "\n\nroot :to => 'home#index'", :after => "# Added via my Template"
#inject_into_file 'config/routes.rb', "\n:root ", :after => "require 'rspec/rails'"
#route("resources :users") #Inserted at the top of the routes.rb file
...
inject_into_file 'public/javascripts/application.js', :after => "// This file is automatically included by javascript_include_tag :defaults" do
  <<-eos
...
temp_application_html_path = File.join(File.dirname(__FILE__), 'files','temp_application.html.erb')
...
#         <% if user_signed_in? %>
#              Hello, <%#= current_user.first_name %> <%= link_to current_user.first_name, edit_user_path(current_user) %> | <%= link_to "Sign out", destroy_user_session_path %>
#         <% else %>
...
# end
# 
# inject_into_file 'app/views/layouts/application.html.erb', :after => '<%# placeholder %>' do
